        ...
        subProjects: [
          {
            ...
            achievements: [
              ...
              "누적 1,258만 건 댓글 데이터를 수집·처리하는 운영 규모 확보",
          ...
          },
          {
            ...
            achievements: [
              "유료 워크스페이스의 52%가 자동 DM을 설정, 전체 자동 실행 건수의 65%가 유료 워크스페이스에서 발생",
              "플랫폼 핵심 사용 지표로 자리잡아, 활동 유저 판정 기준(크레딧 소비 또는 자동 실행)에 채택",
            ],
          },
          {
          ...
          },
          {
            ...
            problem: "크레딧 소모·리포트 퍼널 같은 자사 고유 지표는 범용 분석 도구로 볼 수 없고, 지표에서 개별 유저까지 추적하려면 자사 DB와 연결이 필요한 상황",
            role: "내부 애널리틱스 어드민을 풀스택으로 단독 설계·구현 (NestJS API + Next.js)",
            contributions: [
              "AWS Athena + S3 기반 이벤트 분석 파이프라인 구축",
              "Acquisition·Retention·Feature Usage를 자사 도메인 기준으로 정의하고 대시보드로 구현",
              "MongoDB 연동 드릴다운으로 집계 지표에서 개별 유저까지 추적",
            ],
            achievements: [
              "상용 분석 도구 없이 자체 구축 — 기존 운영 클러스터에 얹어 전용 인프라 증설 없이 월 $10 미만으로 운영",
              "인사이트 v2·리포트 프로젝트의 KPI 측정 기반으로 활용",
            ],
          },
        ],
      },

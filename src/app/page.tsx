        ...
        subProjects: [
          {
            ...
            contributions: [
              "팀원과 AI가 함께 일하는 문서 허브를 설계·구축 — 기획·리뷰·개발·QA·회고 기록을 하나의 저장소 규약으로 통합하고, 완료 프로젝트는 history로 이관해 담당자가 바뀌어도 의사결정 맥락이 복원되는 구조 확보 (기획·개발·디자인 5명이 상시 사용)",
              "허브 위에 자동화 계층 구축 — CLAUDE.md 계층과 커스텀 Skill·Agent로 수집 사이클러(cron), MongoDB 적재, Jira 양방향 동기화, PRD·QA 검증 게이트 운영",
          ...
          },
          {
            ...
            problem: "유저 행동을 분석하려면 상용 프로덕트 애널리틱스 도구를 도입해야 하는데, 구독 비용이 지속 발생하는 데다 크레딧 소모·리포트 퍼널·플랜 전환 같은 자사 고유 지표는 범용 도구로 표현되지 않는 상황",
            role: "내부 애널리틱스 어드민을 풀스택으로 단독 설계·구현 (NestJS API + Next.js)",
            contributions: [
              "AWS Athena + S3 데이터레이크에 행동 이벤트를 적재·조회하는 자체 분석 파이프라인 구축",
              "Acquisition·Retention·Feature Usage를 자사 도메인 기준으로 정의하고 대시보드로 구현",
              "MongoDB 연동 유저 조회·드릴다운으로 집계 지표에서 개별 유저까지 추적 가능한 구조 확보",
            ],
            achievements: [
              "상용 애널리틱스 도구 도입 없이 자체 트래킹·분석 환경 확보 — 구독 비용 없이 자사 고유 지표를 직접 정의",
              "리텐션·유저 세그먼트·이벤트 로그를 상시 조회 가능한 내부 도구로 확보, 이후 인사이트 v2·리포트 프로젝트의 KPI 측정 기반으로 활용",
            ],
          },
        ],
      },
